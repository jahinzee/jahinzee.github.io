...
I call '*shrimp got drimp*' – an old meme edit I made of [Goku Drip][][^1] and a shrimp:

![A screenshot of a Paint.NET window, with an image of a shrimp wearing Goku
Drip.](media/digital-dongles/paint-dot-net.png)
...
![A screenshot of a Pinta window, with an empty canvas with the text "...where
shrimp?".](media/digital-dongles/pinta-noshrimp.png)

Except, now we have another problem, **how do we read Paint.NET files in Pinta?**

## Step 2: Migrating the file formats

Pinta doesn't support Paint.NET files[^2], nor does Paint.NET support the [OpenRaster format][] that
Pinta uses. In the abstract, this problem has a very simple solution - find some way to convert one
to the other!
...
versions of the Paint.NET format. Given all of this, I thought I would have to try my hand at
reverse engineering the Paint.NET format – something which I have no experience with.

But eventually, I found something very interesting: [LazPaint][].
...
![A screenshot of a LazPaint window, with the Goku Drip shrimp
image.](media/digital-dongles/lazpaint.png)
...
![A screenshot of a Pinta window, with the Goku Drip shrimp
image.](media/digital-dongles/pinta-shrimp.png)
...
as a stopgap, Coca162's [PdnBatchConverter][] might be worth looking at, although I haven't tried
it.
...
![A screenshot of a newer Pinta window with a unified headerbar and no
menubar.](media/digital-dongles/pinta-new.png)
...
the same time I discovered [PhotoGIMP][], which is a set of config files and plugins that make
GIMP's UX closer to Photoshop. But as someone who's only used Photoshop very infrequently (and is
allergic to SaaS), PhotoGIMP was a lot more approachable to me. It wasn't quite Paint.NET, but it's
much better than stock GIMP.
...
If you haven't already, consider checking out [Local-First software][], and converting your stuff to
open formats if you can – be it OpenRaster, [OpenDocument][], Markdown, Vorbis, or whatever suits
you. It might take some time and a bit of creativity, but it'll be worth it for the power and
freedom you have over your own data. 
...
[^2]: There was a third-party plugin that let you import Paint.NET files, but it wasn't compatible
      with Pinta 2.0. See [the GitHub discussion about it][pdn-pinta-discussion].

[^3]: The menu bar and the ability to run w/o the unified header bar did return in [a future
      update][pdn-menubar-update], but by then I'd already moved on (womp womp).

[Goku Drip]: https://knowyourmeme.com/memes/goku-drip
[OpenRaster format]: https://en.wikipedia.org/wiki/OpenRaster
[LazPaint]: https://lazpaint.github.io/
[PdnBatchConverter]: https://github.com/Coca162/PdnBatchConverter
[PhotoGIMP]: https://github.com/Diolinux/PhotoGIMP
[Local-First software]: https://www.inkandswitch.com/essay/local-first/
[OpenDocument]: https://www.libreoffice.org/discover/what-is-opendocument/
[pdn-pinta-discussion]:
    https://github.com/PintaProject/Pinta/discussions/827#discussioncomment-9612027
[pdn-menubar-update]: https://github.com/PintaProject/Pinta/issues/781